...
// originally from:  https://github.com/TheMechanicX32/BSP-Swift-dungeon-Generator
//
// TODO: rewrite
...
class Leaf : Codable {
    ...
    // tweaking these values can cause app to not generate map (hang)
    ...
    public var hallways = [MapGeometryObject]()
    ...
    public func createRooms() {
        ...
        if leftChild != nil || rightChild != nil {
        ...
        } else if (Int.random(in: 0..<100) > 25) && (DungeonMap.currentNumberRooms < UserDefaults.standard.integer(forKey: "maxNumberRooms")) {
            ...
            DungeonMap.currentNumberRooms = DungeonMap.currentNumberRooms + 1
        }
    }
    public func createHall(left:Room, right:Room) {
        ...
        let hallwayWidth = 0//Bool.random() ? 0 : 1
        if Bool.random() {
            // Horizontally first, then vertically:
            // From point1 to min(point2.x, point1.y):
            hallways.append(MapGeometryObject(X: min(point1.x, point2.x), Y: point1.y,
                                              W: Int(abs(point1.x - point2.x)), H: hallwayWidth))
            // From (point2.x, point1.y) to point2:
            hallways.append(MapGeometryObject(X: point2.x, Y: min(point1.y, point2.y),
                                              W: hallwayWidth, H: Int(abs(point1.y - point2.y))))
        } else {
            // Vertically first, then Horizontally:
            // From point1 to min(point1.x, point2.y):
            hallways.append(MapGeometryObject(X: point1.x, Y: min(point1.y, point2.y),
                                              W: hallwayWidth, H: Int(abs(point1.y - point2.y))))
            // From (point1.x, point2.y) to point2:
            hallways.append(MapGeometryObject(X: min(point1.x, point2.x), Y: point2.y,
                                              W: Int(abs(point1.x - point2.x)), H: hallwayWidth))
        }
    }
}
